...
async def run(request: EscalationRequest) -> EscalationResponse:
    ...
    Launches a Chromium browser (headless by default), navigates to
    ``TIER3_TARGET_URL``, submits the escalation context as a message,
    waits for the AI to respond, and extracts the response text.
    ...
    - ``TIER3_INPUT_SELECTOR`` – CSS selector for message input
    - ``TIER3_SEND_SELECTOR`` – CSS selector for send button
    - ``TIER3_RESPONSE_SELECTOR`` – CSS selector for response container
    - ``TIER3_STOP_SELECTOR`` – CSS selector for stop-generation button
...
async def _browser_session(prompt: str) -> str:
    ...
    async with async_playwright() as pw:
        if _CHROME_PROFILE_DIR:
            # Use a persistent context so the user stays logged in
        ...
        try:
            ...
            try:
                ...
                # Fallback: just wait for a response container to become visible
                await page.locator(_RESPONSE_SELECTOR).last.wait_for(
                    state="visible", timeout=_RESPONSE_TIMEOUT_MS
                )

            # Extract text from the last response container
