"""SQLite-backed audit log for agent events."""
...
class AuditLog:
    """Async interface to the SQLite audit log table.
    ...
    async def log(self, entry: AuditEntry) -> None:
        """Persist an AuditEntry to the SQLite audit table.
    ...
    async def query(self, task_id: str) -> list[AuditEntry]:
        ...
        Returns:
            List of AuditEntry objects ordered by creation time.
        """
        async with _lock:
            conn = _get_conn()
            rows = conn.execute(
